...
struct Well* createWell() {
  struct Well* w = malloc (sizeof (struct Well));
  w->mutex = uthread_mutex_create();
  w->little = uthread_cond_create(w->mutex);
  w->big = uthread_cond_create(w->mutex);
  w->count = 0;
  w->entered = 0;
  w->endianness = 0;
  return w;
...
int main (int argc, char** argv) {
  ...
  for(int i = 0; i < NUM_PEOPLE; i++)
	pt[i] = uthread_create(occupant, NULL);
